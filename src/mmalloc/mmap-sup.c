...
PTR
__mmalloc_mmap_morecore (struct mdesc *mdp, size_t size)
{
  PTR result = NULL;
  ...
  if (size == 0)
    ...
    /* FIXME:  Test results of lseek() and write() */
	  (void) lseek (mdp -> fd, foffset + mapbytes - 1, SEEK_SET);

	  int nbytes = write (mdp -> fd, &buf, 1);
    if (nbytes == -1)
      perror("Cannot read the file descriptor.");
